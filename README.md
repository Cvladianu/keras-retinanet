The text below, along with the folders "tests", "images", "examples", "snapshots", "keras-retinanet" and with the files "setup.cfg", "setup.py", "LICENCE", ".travis.yml" DO NOT belong to me, Cosmin Vladianu, are not part of my work or a result of my work. Instead, they are attributed to the github user fyzr (https://github.com/fizyr), as well as the RetinaNet model implementation

The jupyter notebook contains the code generated for the project. 

The folder all_annotations contains most of the annotations generated by the various runs of RetinaNet, and can be used to create statistics and/or verify the results from my final report. 

The folder Image_data_+_altitude, contains the metadata from 2 datasets from the SUAAVE project. The second one, with index 1, was used to extract the drone's altitude

The file runconfig.ini contains the configurations of the code used for the project. The ones marked with [DEFAULT] should be copied and used as templates
